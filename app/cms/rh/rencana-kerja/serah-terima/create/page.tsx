...
const useSerahTerimaForm = create<FormState>((set) => ({
  id: "",
  nomor_bast: "",
  kegiatan: "",
  kontrak: "",
...
const serahTerimaFormSchema = z.object({
...
export default function SerahTerimaForm() {
  const form = useSerahTerimaForm();
  const [errors, setErrors] = useState<Record<string, string>>({});

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const result = serahTerimaFormSchema.safeParse(form);
    if (!result.success) {
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <div>
            <Breadcrumbs items={[{ label: "Rencana Kerja", href: "#" }, { label: "Serah Terima", href: "#" }, { label: "Buat Data" }]} />
            ...
            <p className="text-sm text-base-gray">Form untuk membuat data Serah Terima</p>
          </div>
          <div className="pt-4 flex justify-end gap-2">
            <Button type="submit" variant="green" onClick={handleSubmit}>
              <IconDeviceFloppy /> Simpan
            </Button>
            <Link href="/cms/rh/rencana-kerja/serah-terima">
